...
public class RuleServiceImpl implements RuleService {
    ...
    public void generateAndSaveRules() {
        ...
        String[][] data = {
                {"Vehicles", "Golf 8", "Peugeot 308", "Tesla Model 3", "BMW X5", "Audi A4", "Mercedes C Class", "Ford Fiesta", "Chevrolet Bolt", "Nissan Leaf", "Honda Accord", "Toyota Corolla", "Mazda CX-5", "Jeep Wrangler", "Subaru Outback", "Hyundai Elantra", "Kia Soul", "Volkswagen Passat", "Chevrolet Malibu", "Ford Mustang", "Dodge Charger"},
                {"Real Estate", "Apartment NYC", "Villa LA", "House SF", "Office Building", "Warehouse", "Condo Miami", "Townhouse Chicago", "Penthouse Dallas", "Ranch Denver", "Cottage Boston", "Bungalow Houston", "Duplex Atlanta", "Studio Las Vegas", "Loft Seattle", "Farmhouse Nashville", "Mansion San Diego", "Row House Philadelphia", "Cabin Phoenix", "Estate San Antonio", "Terraced House Columbus"},
                {"Agricultural", "Tractor", "Combine Harvester", "Plow", "Seeder", "Irrigation System", "Sprayer", "Baler", "Planter", "Harvester", "Mower", "Cultivator", "Disc Harrow", "Threshing Machine", "Grain Dryer", "Fertilizer Spreader", "Agricultural Drone", "Seed Drill", "Tillage Machine", "Livestock Trailer", "Chisel Plow"},
                {"Industrial", "CNC Machine", "3D Printer", "Conveyor Belt", "Forklift", "Packaging Machine", "Lathe", "Milling Machine", "Drill Press", "Welding Machine", "Grinder", "Hydraulic Press", "Injection Molding Machine", "Laser Cutter", "Plasma Cutter", "Water Jet Cutter", "Robotic Arm", "Automated Guided Vehicle", "Industrial Oven", "Mixing Machine", "Assembly Line"},
                {"Medical", "MRI Scanner", "X-ray Machine", "Ultrasound Machine", "ECG Machine", "Surgical Robot", "Ventilator", "Defibrillator", "Dialysis Machine", "Infusion Pump", "Pacemaker", "CT Scanner", "PET Scanner", "Anesthesia Machine", "Heart-Lung Machine", "Incubator", "Patient Monitor", "Blood Gas Analyzer", "BiPAP Machine", "EEG Machine", "Centrifuge"}
        };

        // Attributes for each category
        String[][] vehicleAttributes = {{"Age", "Fuel Consumption", "Color", "Mileage", "Transmission", "Engine Size", "Horsepower"}};
        String[][] realEstateAttributes = {{"Location", "Size", "Price", "Bedrooms", "Bathrooms", "Year Built", "Lot Size"}};
        String[][] agriculturalAttributes = {{"Brand", "Model", "Condition", "Year", "Usage Hours", "Capacity", "Fuel Type"}};
        String[][] industrialAttributes = {{"Manufacturer", "Warranty", "Capacity", "Power", "Dimensions", "Weight", "Material"}};
        String[][] medicalAttributes = {{"Brand", "Year", "Condition", "Warranty", "Model", "Dimensions", "Weight"}};

        int totalRules = 40;

        // Calculate how many rules to generate for each category
        int rulesPerCategory = totalRules / data.length;

        // Generate and save rules
        for (int j = 0; j < data.length; j++) {
            String[] categoryData = data[j];
            ...
            for (int i = 1; i <= rulesPerCategory; i++) {
                String ruleName = categoryData[i % (categoryData.length - 1) + 1];

                Rule rule = new Rule();
                rule.setName(ruleName);
                rule.setDescription("Description for " + ruleName);
                ...
                switch (categoryName) {
                    case "Vehicles":
                        ruleAttributes = generateAttributes(rule, vehicleAttributes[0]);
                        break;
                    case "Real Estate":
                        ruleAttributes = generateAttributes(rule, realEstateAttributes[0]);
                        break;
                    case "Agricultural":
                        ruleAttributes = generateAttributes(rule, agriculturalAttributes[0]);
                        break;
                    case "Industrial":
                        ruleAttributes = generateAttributes(rule, industrialAttributes[0]);
                        break;
                    case "Medical":
                        ruleAttributes = generateAttributes(rule, medicalAttributes[0]);
                        break;
                    default:
    ...
    private List<RuleAttribute> generateAttributes(Rule rule, String[] attributeNames) {
        List<RuleAttribute> ruleAttributes = new ArrayList<>();
        Random random = new Random();

        // Generate percentages that sum to 100
        BigDecimal[] percentages = generatePercentages(attributeNames.length);
        ...
        for (int i = 0; i < attributeNames.length; i++) {
            ...
            ruleAttribute.setRule(rule);
            ruleAttribute.setAttribute(attribute);
            ruleAttribute.setPercentage(percentages[i].doubleValue());
            ruleAttribute.setValue(0.0);  // Attribute value set to 0
            ruleAttributes.add(ruleAttribute);
        }
    ...
    private BigDecimal[] generatePercentages(int numAttributes) {
        BigDecimal[] percentages = new BigDecimal[numAttributes];
        BigDecimal total = BigDecimal.valueOf(100);

        for (int i = 0; i < numAttributes - 1; i++) {
            BigDecimal percentage = BigDecimal.valueOf(Math.random() * total.doubleValue())
                    .setScale(2, RoundingMode.HALF_UP);
            percentages[i] = percentage;
            total = total.subtract(percentage);
        }
        percentages[numAttributes - 1] = total.setScale(2, RoundingMode.HALF_UP);
